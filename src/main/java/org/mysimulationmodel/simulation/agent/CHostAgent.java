...
public final class CHostAgent extends IBaseAgent<CHostAgent>
{
    ...
    private double m_randomization;
    //only for individual calibration
    private int m_strategy;
    private IBaseRoadUser m_ped; // till here
    //CGame parameter end
    ...
    final void perceive()
    {
        ...
        m_env.getCarinfo().stream()
                ...
                .forEach( i ->
                {
                    ...
                    // all the time except individual calibration
                            ...
                            CVector.distance( i.getPosition(), k.getPosition() ) <= m_distance ).forEach( j ->
                    {
                        ...
                        if ( ( l_angle2 <= 8 || l_angle2 >= 352 ) && !i.getCurrentlyPlayingWith().contains(j))
                        {
                            l_pedinfront.add(j);
                        }
        ...
        l_viewedroadusers.forEach( (i,j) ->
        {
            l_viewedroadusers.forEach( (k,l) ->
            {
                ...
                { //System.out.println( i.getname()+" in here "+k.getname() );
                    j.add( 1, k);
                    l.clear();
                }
            });
    ...
    public void triggerAgents( )
    {
        ...
        {
            //multiple car to ped interaction for road zone
            //m_env.m_playedgame = 1; // here, in normal situation
                    ...
                    && !j.getCurrentlyPlayingWith().contains( i ) && !l_closedlist.contains( i ) )
            {
                ...
                if ( m.size()>1 && m.get(1).getCurrentBehavior() != 1 )
                    {
                        //System.out.println(" multiple conflict car name: "+m.get(1).getname()+ " "+j.getname());
                ...
                if ( !i.get_competitiveUserList().isEmpty() && !l_strategyset.isEmpty() )
                {
                    ...
                    // in case of multiple conflict; .stream().filter(l-> !m_env.getCarbyName().get(l).equals(i))
                    if(m.size()>1)
                    i.getCurrentlyPlayingWith().stream().filter( s -> s.getType()== 1 && s.getCurrentBehavior() == 0 && s.getCurrentlyPlayingWith().contains(m.get(1)) )
                            .forEach(
                                    x ->
                                    {
    ...
    private ArrayList<Integer> getstrategies( final IBaseRoadUser p_self, final IBaseRoadUser p_other, final int p_gametype, final int p_row, final int p_column )
    {
        ...
        if ((l_angle < 22 && l_angle >= 0 ) || l_angle > 338)
        l_angle = 8;
        else if ( (l_angle <= 43 && l_angle >= 22 ) || (l_angle <= 338 && l_angle >= 318 ))
        l_angle = 7;
        else if ((l_angle <= 65 && l_angle > 43 ) || (l_angle < 318 && l_angle >= 295 ))
        l_angle = 6;
        ...
        double[][][] l_matrix = CGame.payoffMatrixCalculationCartoPed( 11, 11, 3, 2,1,(p_self.getSpeed())/m_pixelpermeter, l_angle, p_other.getSpeed()/m_pixelpermeter,
            p_self.get_competitiveUserList().size(), l_stopped, l_distance/m_pixelpermeter);
    ...
    public void updateParameter(double p_distance, double p_collisionchekingfactor, double p_randomization )
    {
    ...
    public void updateParameter(double p_distance )
    {
        m_distance = p_distance;
    }

    public void updateParameter(double p_distance, double p_collisionchekingfactor, int p_randomization, int p_strategy, String p_uid )
    {
        m_distance = p_distance;
        m_collisionchekingfactor = p_collisionchekingfactor;
        m_strategy = p_strategy;
        m_ped = m_env.getPedestrainyName(p_uid);
    }

    public void updateParameter(double p_collisionchekingfactor, int p_randomization)
    {
        m_collisionchekingfactor = p_collisionchekingfactor;
        m_randomization = p_randomization;
    }
