...
public class IBaseRoadUser extends IBaseAgent<IBaseRoadUser>
{
    ...
    //update variables
    public IBaseRoadUser updateParameter( double p_pedtopedsigma, double p_pedtocarsigma, double p_pedtocardistance,
                                          double p_pedlamda, double p_pedtopedforce, double p_pedtocarforce,
                                          double p_pedtocaraccdistance, double p_cartocardistance )
    {
        m_acceleration = 0.30*m_pixelpermeter;
        //m_pedacceleration = 0.05*m_pixelpermeter;

        m_decelerationcartocar = 3*m_pixelpermeter;
        m_decelerationpedtocar = 3*m_pixelpermeter;
        m_pedlamda = p_pedlamda;
        m_repulsefactorpedtoped = p_pedtopedforce;
        ...
        m_cartopeddistance = 7.8*m_pixelpermeter;
        m_pedtocaraccelarationdistance = p_pedtocaraccdistance;
        return this;
    }

    public IBaseRoadUser updateParameter( double p_pedtopedsigma, double p_pedtocarsigma, double p_pedtocardistance,
                                          double p_pedlamda, double p_pedtopedforce, double p_pedtocarforce,
                                          double p_cartocardistance, String p_uid )
    {   name = p_uid;
        m_acceleration = 0.30*m_pixelpermeter;
        //m_pedacceleration = 0.05*m_pixelpermeter;
        ...
        m_cartocardistance = p_cartocardistance;
        m_pedtocardistance = p_pedtocardistance;
        m_pedtocaraccelarationdistance = p_pedtocardistance;
        return this;
    }

    public IBaseRoadUser updateParameter( double p_pedtocarsigma, double p_pedtocardistance,
                                          double p_pedlamda, double p_pedtopedforce, double p_pedtocarforce,
                                          double p_pedtocaraccdistance, double p_cartocardistance )
    {
        m_acceleration = 0.30*m_pixelpermeter;
    ...
    {
          /*
        //these code fragments are only used for individual calibration
        ...
        final double l_slowingRadius = this.getM_radius()/2 ;//+ ( 0.6 * 1.25 );
        double l_acefactor;
        if (this.getType() == 1)
        {
            m_acceleration = (this.m_currentbehavior == 0 || (this.m_currentbehavior == 2)) ? 0.01*m_pixelpermeter:
                    (this.m_currentbehavior == 3 && m_speed <= 0.204)? 0.3*m_pixelpermeter: 0.0;//*m_pixelpermeter;
            //l_acefactor = 0.0;
            l_acefactor = (this.m_currentbehavior == 0) ? 0.01*m_pixelpermeter: 0.0;
        }
        else
        {
            m_acceleration = (this.m_currentbehavior == 0) ? 0.2*m_pixelpermeter:
                    (this.m_currentbehavior == 3 && m_speed <= 0.204)? 0.35*m_pixelpermeter: 0.1*m_pixelpermeter;
            l_acefactor = (this.m_currentbehavior == 0) ? 0.2: 0.0;
            //System.out.println( m_acceleration +" c "+this.m_currentbehavior+ " c "+m_speed);
        }

        if ( this.m_route.size() > 0 )
        {    //add if intermediate point is passed without reaching it, delete it from route list
            if ( l_distance <= m_pixelpermeter )
            {
               ...
               /* this.trigger(
                ...
                m_acceleration = m_acceleration + l_acefactor*m_pixelpermeter <= m_pixelpermeter/2 ? m_acceleration + l_acefactor*m_pixelpermeter : m_acceleration ;
            ...
            else
            {
                double l_speed = this.getVelocity().length() + m_acceleration <= m_maxspeed ? this.getVelocity().length()
                        + m_acceleration : m_maxspeed;
                m_acceleration = m_acceleration + l_acefactor*m_pixelpermeter <= m_pixelpermeter/2 ? m_acceleration + l_acefactor*m_pixelpermeter : m_acceleration ;
                // System.out.println("cs "+ l_speed+" "+m_acceleration);
                this.accelaration( l_speed );
            }
        }
        else
    ...
    {
        /* // this fragments of codes are only for individual calibration
        if(this.getname().startsWith("v"))
        {
            ArrayList<String> l_temp =m_realdata.get( new StringBuffer(new StringBuffer(String.valueOf(m_env.getCurrentCycle()))
                    .append(this.getname()).append(m_env.getScenario())).toString());
            if( l_temp != null )
        ...
        if( CVector.distance( this.getPosition(), this.getCurrentlyPlayingWith().get(0).getPosition()) <= m_cartopeddistance_2 )
        {
            m_speed = this.m_velocity.length();
        ...
        if (l_test != null && this.getCurrentlyOvertakingCar() != l_test)
        {
            if( CVector.distance( this.getPosition(), l_test.getPosition() ) <= m_cartopeddistance)
            {
                m_speed = this.m_velocity.length();
                this.m_velocity = CVector.scale( 0.2, CVector.normalize( CVector.add( this.m_velocity, l_velocity ) ) );
                return this;
    ...
    private IBaseRoadUser decelerating()
    {
        double l_distance = CVector.distance( this.m_position, this.getCurrentlyPlayingWith()
                .get(this.getCurrentlyPlayingWith().size()-1).getPosition()) - m_cartopeddistance;
     ...
     * not using currently
     * */
    //need to check for accuracy and not used currently
    @IAgentActionFilter
