...
public final class CVector
{
    ...
    public static Vector2d directionrotation2( final Vector2d p_currentdirection, final double p_angle )
    {
        System.out.println( "direction "+ p_currentdirection );
        return new Vector2d( p_currentdirection.x * Math.sin( p_angle ) + p_currentdirection.y * Math.cos( p_angle ),
                p_currentdirection.x * Math.cos( p_angle ) - p_currentdirection.y * Math.sin( p_angle ) );
    }
