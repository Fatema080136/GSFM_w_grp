...
public class Genetic_Algorithm_indiv
{
    ...
    {   //m_realdata.forEach((k,m)-> System.out.println(k));
        ...
        while(true) {
            ...
            if (gen==30)
            {
                System.out.println(m_uid+ " s_id "+ m_sid +" finish "+  population.get(population.size()-1).getScore()+" "+
                        population.get(population.size()-1).chromo);
                break;
            }
    ...
    private static class chromosome implements Comparable<chromosome>
    {
        ...
        private double getRandomGene(int gene)
        {
            ...
            if(gene == 1 )
                return ran.nextInt(7)+ 3+Precision.round(ran.nextDouble(),2);
            //if(gene == 2 )
              //  return ran.nextInt(6)+6+Precision.round(ran.nextDouble(),2);
            if(gene == 2)
                return Precision.round(ran.nextDouble(),2);
            return ran.nextInt(12)+6+Precision.round(ran.nextDouble(),2);
        }
            ...
            input.forEach(
                    (p,q) ->
                    {
                        ...
                        eachScenarioResult = CMain_GA.runSimulation(
                                this.chromo.get(0), this.chromo.get(1), 8, this.chromo.get(2), m_sid, m_uid);
